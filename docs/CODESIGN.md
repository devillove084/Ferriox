...
> block-level KV selection with `b_K = 32` compressed entries and `κ = 16`
> selected blocks. A separate `b_K = 1, κ = 512` profile reproduces the
...
`κ` routing blocks per query. The initial performance profile uses
`b_K = 32, κ = 16`, corresponding to a 128-source-position routing stride and a
maximum of `κ b_K = 512` compressed entries per query. Setting
`b_K = 1, κ = 512` reproduces the per-entry selector of Ferriox's deterministic
...
partition-merge top-`κ`. The max-pool profile still evaluates every legal
...
separate tagged sliding-window entries. A temporary prefix top-`κ` is never fed
...
1. **Preserve the V4 computation outside routing.** Match compressed KV
   ...
   explicit `b_K = 1, κ = 512` compatibility profile.
...
7. **Measure the whole pipeline.** Benchmark compression/projection, entry
   scoring, block max, top-`κ`, reverse-index construction, CoreAttn, backward,
...
- Ferriox does **not** accumulate attention for blocks that exist only in a
  temporary prefix top-`κ`.
- Ferriox does **not** materialize MSA/FSA-style full
  `[κ, S, H, c]` output or `dQ` partial buffers at million-token scale.
...
- Ferriox does **not** differentiate through hard top-`κ`. The Ferriox block
...
|---|---|
| `B` | Batch size |
| `S` | Sequence length of one padded tensor |
| `L_b` | Valid sample length for batch item `b` |
| `D` | Model hidden dimension |
| `d_c` | Query latent dimension |
| `c` | CoreAttn query/key/value dimension |
| `c_I` | Lightning-indexer head dimension |
| `H` | Number of CoreAttn query heads |
| `H_I` | Number of indexer query heads |
| `m` | Compression ratio |
| `T_b = floor(L_b / m)` | Number of complete compressed entries for sample `b` |
| `k_V4` | V4 compatibility-profile top-k over individual compressed entries |
| `b_K` | Number of consecutive compressed entries in one routing block |
| `κ` | Maximum selected routing blocks per query |
| `K = κ b_K` | Maximum expanded compressed-entry budget per query |
| `R_b = ceil(T_b / b_K)` | Number of physical routing blocks for sample `b` |
| `w` | Sliding-window size |
| `c_S` | Query chunk size used by the indexer driver |
| `c_T` | Compressed-key chunk size used by the indexer driver |
| `g_A` | CoreAttn heads processed together by one CTA |
...
The V4-Flash rows below come from [5]. The `b_K`, `κ`, `K`, and routing-stride
...
| `c_I` | 128 |
| `m` | 4 |
| V4 entry top-k `k_V4` | 512 |
| Ferriox routing-block entries `b_K` | 32 |
| Ferriox selected blocks `κ` | 16 |
| Ferriox expanded entry budget `K = κb_K` | 512 |
| Routing stride in source positions `m b_K` | 128 |
| `w` | 128 |
| output groups `g` | 8 |
| grouped intermediate width `d_g` | 1024 |
...
### 3.1 Why Prefix Top-`κ` Cannot Be Interleaved with Exact Attention

A streaming block top-`κ` is correct because the final heap after all routing
blocks equals the global top-`κ`. Its *intermediate* heaps are not final.
...
immediately attends to temporary top-`κ` blocks is not equivalent to attention
on the final block top-`κ`.

An exact implementation must do one of the following:

- finalize block top-`κ`, then load the selected CoreAttn blocks; or
...
- **Chunked inference path:** Stage A and Stage B pipeline by query slab and reuse
  a bounded `[c_S, κ]` block-ID buffer.
...
- complete every routing-block maximum before that block enters top-`κ`;
- preserve FP32 reduced entry scores, block maxima, and running top-`κ` scores in
  the correctness path;
- include every legal block when fewer than `κ` blocks exist;
- use the deterministic entry and block comparators from Section 2.5;
- emit exactly `κ` block IDs per query, padded with `-1` after selection;
...
one score per routing block, and maintains only `κ` running candidates.
...
schedule query chunks [q0, q1) through at most P_slot workspace slots:
    ...
    run_score = -infinity [B, q1-q0, κ] FP32
    ...
    append invalid (false, -infinity, -1) sentinels to reach κ slots
...
chunks and submit a block to top-`κ` exactly once; treating two partial maxima as
two candidates is incorrect.

For `b_K = 1, κ = 512`, Algorithm 1 reduces to the V4-compatible StreamIndex
partition-merge algorithm. For `b_K = 32, κ = 16`, entry QK arithmetic is
...
merge buffers are budgeted separately but have the same `O(c_S κ)` scale.
...
immediately select local block top-`κ`, and write only local block candidates.
...
not top-`κ` state, become the main resource gates. State remains CTA-distributed,
...
- A chunk with fewer than `κ` legal blocks contributes all of them.
...
The fixed `[B, S, κ]` block slots are the query-major CSR-equivalent view. For
`κ = 16`, the row offset is implicit (`row_start = query_id * κ`), so no CSR
...
`κ` blocks, keeps one sink-aware online-softmax state, and writes the final output
...
`Obuf[κ,S,H,c]` at `κ=16, S=1M, H=64, c=512` is approximately 976.6 GiB; FP32 is
about 1.91 TiB, and `LSEbuf[κ,S,H]` adds 3.81 GiB. Therefore KV-outer forward must
...
2.  **Tie-break and top-κ selector semantics.**
    ...
    theoretical; a different tie-break or top-κ order can produce a
...
published kernel's actual tie-break / top-κ order into the checkpoint
...
Top-`κ` is discontinuous. A small score error can swap blocks around the
`κ`/`κ+1` boundary, and max pooling also makes the winning entry inside each
...
- block recall, Jaccard similarity, and the `κ`/`κ+1` block-score margin;
...
entry reductions, block maxima, and running top-`κ` scores in FP32. Lower
...
H=64, c=512, b_K=32, κ=16, K=512, w=128,
```
...
This excludes indexer ReLU/head reduction, block max/top-`κ`, scalar online
...
| Reverse query handles | up to `Sκ` int32 | 61.04 MiB |
...
- running block candidates are `O(c_Sκ)`, not `O(c_SK)`;
...
7. perform local block top-`κ` or write bounded block-score scratch.
...
### 8.3 Block-Max and Top-`κ` Mapping

At `κ = 16`, one query row's FP32 score/block-ID state is 128 bytes rather than
...
- keep each lane's small local block top-`κ`, then merge at warp/CTA scope;
- use a CTA-distributed heap or sorting network for `κ = 16`;
- retain the bounded global running state as the portable baseline.

MSA reports a specialized `B_K=128, κ=16` selector outperforming general radix
...
- FP32 reduced entry score, routing-block maximum, and running top-`κ` score;
...
- indexer QK may use the V4 FP4/QAT path, but reduced scores, block maxima, and
  top-`κ` boundaries remain FP32 unless block-boundary recall experiments justify
  otherwise;
...
- finalized block IDs `[B,S,κ]`, including `-1` sentinels;
...
`64 * 511,475,200 = 32,734,412,800`. The capacity model below is a conservative,
...
for causal packed sequences. Configuration validation requires `b_K>0`, `κ>0`,
checked multiplication for `K=κb_K`, block-aligned baseline `c_T`, and range-safe
...
- MSA's post-score block max, exp-free top-`κ`, detached KL, and block execution
  [12];
- FlashMoBA's query-major to key-block-major transpose and atomic backward [16].

The hypothesis is deliberately narrow:

> `b_K=32, κ=16` can preserve an acceptable fraction of V4-compatible attention
...
```text
b_K=1, κ=512  == materialized V4-compatible entry selector
b_K=32,κ=16   == materialized Ferriox block reference
```
...
- query chunk `c_S`;
- entry chunk `c_T`, constrained to `c_T % b_K == 0` initially;
- indexer-head group;
- local block-score tile;
- selection primitive for `κ=16`;
...
- slab-local projection/attention fusion so full `q_core` and `o_core` need not
  coexist at one million tokens.

The Blackwell path must include a generated-code report for every candidate
`g_A`: registers/thread, SMEM/CTA, TMEM, resident CTAs, TMA transactions, UMMA
...
For each `b_K in {1,16,32,64,128}` and `κ` chosen to keep `κb_K` near 512,
...
`b_K=32,κ=16` remains the default only if it lies on the measured
...
with Stage B of slab `n`, but Stage B never consumes prefix top-`κ`.
...
classes of dependency must be resolved before local top-κ can execute.

**Operand visibility.**
Each rank computing local top-κ against its shard of compressed index keys
...
and topology; "exchange at most κ pairs per query" describes only the
...
1. each rank computes local block top-κ;
2. ranks exchange at most κ `(score,global_block_id)` pairs per query
   (the total network volume includes rank and topology factors);
3. all ranks apply the same total-order global merge;
4. only the finalized global top-κ is consumed;
...
1. `κ = 1` where a later routing block evicts an earlier prefix winner after the
...
4. `T_legal(t)=0`, fewer than `κ` legal blocks, and post-selection `-1` sentinels.
...
6. A final physical routing block containing fewer than `b_K` complete entries.
...
10. `b_K=1,κ=512` exact selected-set/support parity with the V4-compatible
...
21. Distributed local-top-`κ` plus global merge exactly equals one-rank block
    top-`κ` under ties and NaNs.
...
- exact block-set rate, block recall/Jaccard, and `κ`/`κ+1` margin;
...
- quality versus `b_K` at fixed expanded budget and versus expanded budget at
  fixed `b_K`.
...
- global top-`κ` communication bytes and merge time;
...
`b_K=32,κ=16` admits every legal entry around one block winner and can discard
...
`κ`/`κ+1` block boundary. Precision rollout is gated on winner stability,
...
- **MiniMax Sparse Attention (MSA)** [12] is the closest structural precedent:
  Index Branch, post-score block max, final block top-`κ`, exact Main Branch,
...
- **SageAttention** [8–10] motivates low-precision attention kernels. Ferriox
  separates arithmetic error from selector boundary error because max and top-`κ`
  are discontinuous.
...
- **Predict, Reuse, and Repair (PRR)** [27] predicts likely retrieval blocks,
  ...
  retain predicted extra blocks and therefore is not strict exact top-`κ`; it is
...
2. max-pool complete routing blocks and finalize deterministic top-`κ`;
...
The initial `b_K=32,κ=16` profile keeps the maximum 512-entry CoreAttn budget,
